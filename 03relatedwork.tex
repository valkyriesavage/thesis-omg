...
\valkyrie{there are classification problems and regression problems. 
classification can either be based on ad hoc features created by the authors [examples] or by using ML algorithms to learn important features from a rich representation of the user’s action, e.g. by generating a variety of features about spatial and temporal aspects of their performance.}

\valkyrie{one common way is to use supervised machine learning, where many labeled examples are used to train a classifier. [add examples] but this requires training.}

\valkyrie{Also, calling out vision, audio, and capacitive sensing examples separately and explicitly would make sense since you employ those three approaches in your projects as well.}
