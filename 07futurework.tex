...
\valkyrie{In all three projects, the designer specifies higher-level intent “create a slider here, a button there”. (Lamello doesn’t have a design tool, but I think the general vision is there). Then you combine this intent with knowledge of the sensing approach you will take to generate or modify geometry for digital fabrication. Afterwards, there are some manual assembly steps. }
\valkyrie{One vision would be to fully automate this pipeline - the designer expresses their high-level intent in a 2D or 3D modeling environment, and a future multi-material printer with integrated pick-and-place generates a dully working device in a single pass.}

\valkyrie{Could these techniques also be robust enough to create devices for daily use - not just prototypes?}

\valkyrie{you focused exclusively on tangible input devices. What other domains might “Fabbing to Sense” be applied to? What would you have to change in those domains?}
