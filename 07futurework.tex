...
One vision would be to fully automate this pipeline - the designer expresses their high-level intent in a 2D or 3D modeling environment, and a future multi-material printer with integrated pick-and-place generates a dully working device in a single pass.

Could these techniques also be robust enough to create devices for daily use - not just prototypes?

you focused exclusively on tangible input devices. What other domains might “Fabbing to Sense” be applied to? What would you have to change in those domains?}
